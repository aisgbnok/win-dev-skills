...
description: 'Add a Settings page to a WinUI 3 app with grouped settings, toggle switches, theme selection, and persistent storage. Use when asked to add settings, preferences, options, configuration page, or app settings to a WinUI 3 application.'
---

# Journey: Add a Settings Page to a WinUI 3 App

## Quick Reference

1. **Use `NavigationView`** — settings page is a navigation destination triggered by the `Settings` item built into `NavigationView`.
2. **Group related settings** — use `SettingsExpander` (CommunityToolkit) or manual `StackPanel` groups with headers.
3. **Persist every change immediately** — write to `ApplicationData.Current.LocalSettings` (packaged) or a JSON file (unpackaged) on every toggle/selection change.
4. **Load saved values on startup** — read persisted settings in the ViewModel constructor *before* the page binds.
5. **Apply theme at the Window level** — set `((FrameworkElement)Content).RequestedTheme` on the root element, not on individual pages.

---

## When to Use This Skill

- User asks to add a **Settings**, **Preferences**, **Options**, or **Configuration** page.
- User wants **theme switching** (light/dark/system), **toggle preferences**, or an **About** section.
- User needs to **persist user preferences** across app restarts.
