---
name: code-templates
description: 'Pre-built XAML and C# code templates for common WinUI 3 app patterns. Use when building a list-detail view, settings page, login page, dashboard layout, master-detail navigation, data entry form, or any standard app layout pattern. These are verified, copy-paste-ready patterns that minimize iteration.'
---

# Code Templates for WinUI 3

## Quick Reference

- **Always use `{x:Bind}` with explicit `Mode=`** — `OneWay` for display, `TwoWay` for input fields, omit (default `OneTime`) for static labels.
- **Use `[ObservableProperty]` and `[RelayCommand]`** from `CommunityToolkit.Mvvm` — never write boilerplate `INotifyPropertyChanged` by hand.
- **Use `ThemeResource` for all colors and brushes** — never hardcode hex values; this ensures dark/light theme support automatically.
- **Every template is self-contained** — copy the XAML into a `Page`, the C# into a matching ViewModel, and wire up via `Page.DataContext`.
- **Compose templates together** — combine Command Bar + List-Detail, or Dashboard Cards + Empty State, to build full screens quickly.
